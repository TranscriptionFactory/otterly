...
function make_mock_graph_port(): GraphPort {
  return {
    load_note_neighborhood: vi.fn().mockResolvedValue({ center: {}, backlinks: [], outlinks: [], orphan_links: [], stats: {} }),
    load_vault_graph: vi.fn().mockResolvedValue({ nodes: [], edges: [], stats: { node_count: 0, edge_count: 0 } }),
    invalidate_cache: vi.fn().mockResolvedValue(undefined),
    cache_stats: vi.fn().mockResolvedValue({ size: 0, hits: 0, misses: 0, insertions: 0, evictions: 0, hit_rate: 0 }),
  };
}
...
function make_mock_search_port(
  ...
  return {
    find_similar_notes: vi.fn().mockImplementation((_vault_id, note_path) =>
    ...
    get_note_links_snapshot: vi.fn().mockResolvedValue({ backlinks: [], outlinks: [], orphan_links: [] }),
    extract_local_note_links: vi.fn().mockResolvedValue({ outlink_paths: [], external_links: [] }),
    rewrite_note_links: vi.fn().mockResolvedValue({ markdown: "", changed: false }),
    ...
    get_embedding_status: vi.fn().mockResolvedValue({ total_notes: 0, embedded_notes: 0, model_version: "", is_embedding: false }),
...
describe("GraphService.toggle_semantic_edges", () => {
  ...
  it("does not reload when toggling off then on with cached edges", async () => {
    ...
    await service.toggle_semantic_edges();
    const call_count = (search_port.find_similar_notes as ReturnType<typeof vi.fn>).mock.calls.length;

    await service.toggle_semantic_edges();
    await service.toggle_semantic_edges();

    expect((search_port.find_similar_notes as ReturnType<typeof vi.fn>).mock.calls.length).toBe(call_count);
  });
});
